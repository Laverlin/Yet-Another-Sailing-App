...
class CruiseViewDc
{
    ...
    hidden var halfd = 0;
    hidden var toply = 0;
    hidden var bottomly = 0;

    function initialize(screenHeight)
    {
        _screenHeight = screenHeight;
        System.println(_screenHeight);
        maxd = _screenHeight;
        halfd = _screenHeight / 2;
        toply = 70;
        bottomly = 215;
    }
	...
	function PrintTime(dc, time)
	{
		...
		dc.setColor(Settings.ForegroundColor, Settings.BackgroundColor);
        dc.drawText(halfd, 7, Gfx.FONT_MEDIUM, timeString, Gfx.TEXT_JUSTIFY_CENTER);
	}
    ...
    function PrintSpeed(dc, speed)
    {
    	...
    	dc.setColor(Settings.ForegroundColor, Settings.BackgroundColor);
        dc.drawText(_screenHeight / 2 -20, _screenHeight / 2 - 38, Gfx.FONT_NUMBER_HOT, speedString, Gfx.TEXT_JUSTIFY_RIGHT);
        dc.drawText(_screenHeight / 2 -8, _screenHeight / 2 + 24, Gfx.FONT_XTINY, "SOG", Gfx.TEXT_JUSTIFY_RIGHT);
    }
    ...
    {
    	dc.setColor(Settings.ForegroundColor, Settings.BackgroundColor);
        dc.drawLine(0, toply, maxd, toply);
		dc.drawLine(0, bottomly, maxd, bottomly);
		dc.drawLine(halfd, toply, halfd, bottomly);
    }
